...
class editOfferDetails extends StatefulWidget {
  editOfferDetails(
      ...
      required this.idofferdetails,
      required this.offerImage,
      required this.index});
  final String offerTitle;
  final String offerPrice;
  final int idofferdetails;
  String offerImage;
  final int index;

  @override
  State<editOfferDetails> createState() => _editOfferDetailsState();
}

class _editOfferDetailsState extends State<editOfferDetails> {
  ...
  void uploadImage() async {
    ...
    try {
      ...
      firebase_storage.Reference storageRef = firebase_storage
          ...
          .child(
              'item_${widget.idofferdetails}${Random}.jpg'); // Adjust the filename or path as needed
  ...
  Future<void> editDetailsinfo(
      {required String idofferdetails,
      required String offerdetailsName,
      required String price,
      required String image}) async {
    final ipAddress = await getLocalIPv4Address();
    final url = Uri.parse('http://$ipAddress:3000/editofferDetails');
    ...
    try {
      final response = await http.post(
        ...
        body: {
          ...
          if (offerdetailsName.isNotEmpty)
            'offerdetailsName': offerdetailsName
          else
            'offerdetailsName': widget.offerTitle,
          if (price.isNotEmpty) 'price': price else 'price': widget.offerPrice,
          if (image != null) 'image': image else 'image': widget.offerImage,
        },
      );
        ...
        child: GestureDetector(
          ...
          child: ListView(
            children: [
              ...
              buildTextField(
                  S.of(context).title, widget.offerTitle, titleController),
              buildTextField(
                  S.of(context).price, widget.offerPrice, priceController),
              SizedBox(
                height: 30,
              ),
              Row(
                ...
                children: [
                  ...
                  ElevatedButton(
                    onPressed: () async {
                      ...
                      await editDetailsinfo(
                        idofferdetails: widget.idofferdetails.toString(),
                        offerdetailsName: titleController.text,
