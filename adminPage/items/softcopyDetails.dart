...
class softCopyDetails extends StatefulWidget {
  const softCopyDetails({super.key});

  @override
  State<softCopyDetails> createState() => _softCopyDetailsState();
}

class _softCopyDetailsState extends State<softCopyDetails> {
  List<Map<String, dynamic>> softArray = [];
  @override
  void initState() {
    super.initState();
    initializeData();
  ...
  Future<void> initializeData() async {
    try {
      softArray = await getItems("1");
      print('mealArray: $softArray');

      if (softArray.isNotEmpty) {
        setState(() {
          softArray = softArray;
        });
      }
  ...
  Future<List<Map<String, dynamic>>> getItems(String idCategory) async {
    ...
    final String apiUrl = "http://$ipAddress:3000/getItemsSoftware/$idCategory";
                                ...
                                child: Column(
                                  children: [
                                    ...
                                    Center(
                                      child: Text(
                                        "Student Name: " +
                                            softArray[index]["Fname"] +
                                            " " +
                                            softArray[index]["Lname"]
                                                .toString(),
                                        style: TextStyle(
                                          fontSize: 16,
                                          fontWeight: FontWeight.w500,
                          ...
                          child: GestureDetector(
                            onTap: () {
                              _showBottomSheet(
                                  context,
                                  index,
                                  softArray[index]["iditem"].toString(),
                                  softArray[index]["image"]);
                            },
                            child: Container(
                              decoration: BoxDecoration(
                                borderRadius: BorderRadius.circular(24),
                  ...
                  child: ListTile(
                    ...
                    onTap: () async {
                      ...
                      Navigator.push(
                          ...
                          MaterialPageRoute(
                            builder: (context) =>
                                PdfPreviewScreen(pdfFilePath: pdfFilePath),
                          ));
                    },
                  ),
                ),
