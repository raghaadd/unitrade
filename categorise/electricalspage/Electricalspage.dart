...
class Electricalspage extends StatefulWidget {
  final int index;
  final int registerid;
  const Electricalspage({super.key, required this.index,required this.registerid});

  @override
  State<Electricalspage> createState() => _ElectricalspageState();
}

class _ElectricalspageState extends State<Electricalspage> {
  ...
  Future<void> fetchDataFromAPI(int registerId) async {
    ...
    final response = await http
        .get(Uri.parse('http://$ipAddress:3000/electricalspage/$registerId'));
